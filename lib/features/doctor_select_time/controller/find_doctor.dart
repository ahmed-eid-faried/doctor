...
abstract class SelectTimeDoctorController extends GetxController {
  ...
  nextAvailability();
  changeSelected(int index, bool availableTime);
  contactClinic();
...
class SelectTimeDoctorControllerImp extends SelectTimeDoctorController {
  int selectedDay = 0;
  int selectedReminderMeBefore = 0;
  int selectedAvailableTime = 0;
  ...
  changeSelected(int index, bool availableTime) {
    availableTime
        ? (selectedAvailableTime = index)
        : (selectedReminderMeBefore = index);
    update();
  }
